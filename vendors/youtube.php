...
class YoutubeHelper extends Helper {
	var $urls = array(
		'video' 		 => 'http://gdata.youtube.com/feeds/api/videos/%s',
	...
	function getVideo($video){
		if(empty($video)){
			$this->__log(__FUNCTION__, 'Video não informado!');
			return false;
		}
		...
		$url = sprintf($this->urls['video'], $video);
		...
		$return['videos'][] = array(
			'title'			=> current(Set::extract('/Entry/Group/title/value', $xmlArray)),
			'description'	=> current(Set::extract('/Entry/Group/description/value', $xmlArray)),
			'category'		=> current(Set::extract('/Entry/Group/category/label', $xmlArray)),
			'keyword'		=> explode(', ', current(Set::extract('/Entry/Group/keywords', $xmlArray))),
			'player_url'	=> current(Set::extract('/Entry/Group/Player/url', $xmlArray)),
			'duration'		=> current(Set::extract('/Entry/Group/Duration/seconds', $xmlArray)),
			'video_url'		=> current(Set::extract('/Content[isDefault=true]/url', Set::extract('/Entry/Group/Content', $xmlArray))),
			'thumbs'		=> current(Set::extract('/Entry/Group/Thumbnail/.', $xmlArray)),
		);
		print_R($return);exit;
		$this->__cache($cacheKey, $return);
