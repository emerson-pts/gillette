 ...
 */
	var $defaultOptions = array(
		...
		'uploadNameFixed' => '{fieldName}',//força o nome do arquivo enviado
		'uploadNameIncludeDir' => true,//incluir o diretório no campo do nome do arquivo
		...
		'length' => array(
			...
			'minHeight' => 0,
			'maxHeight' => 4096,//0
			'aspectRatio' => 0,//0 não verifica, 1.3 aceita somente 1.3 e array(1.3,1.5) aceita entre 1.3 e 1.5
		),
		'validations' => array()
	);
 ...
 */
	var $defaultValidations = array(
		...
		'AspectRatio' => array(
			'rule' => array('uploadCheckAspectRatio'),
			'check' => true,
			'last' => true
		),
		
		'UploadedFile' => array(
			'rule'	=> array('uploadedFile'),
			'check' => true,
			'last' => true,
		)
	);
 ...
 */
	function __construct() {
		$messages = array(
			...
			'MaxSize' => array(
				'message' => __d('meio_upload', 'The maximum file size (%s) is exceeded.', true)
			),
			'InvalidMime' => array(
			...
			'MinWidth' => array(
				'message' => __d('meio_upload', 'Image width is smaller than minimum allowed (%s pixels).', true)
			),
			'MinHeight' => array(
				'message' => __d('meio_upload', 'Image height is smaller than minimum allowed (%s pixels).', true)
			),
			'MaxWidth' => array(
				'message' => __d('meio_upload', 'Image width is larger than maximum allowed (%s pixels).', true)
			),
			'MaxHeight' => array(
				'message' => __d('meio_upload', 'Image height is larger than maximum allowed (%s pixels).', true)
			),
			'AspectRatio' => array(
				'message' => __d('meio_upload', 'Image aspect ratio (width/height: %s) of image is not allowed.', true)
			),
			'UploadedFile' => array(
				'message' => __d('meio_upload', 'The sent file does not seen uploaded file.', true)
			),
		);
 ...
 */
	function setup(&$model, $settings = array()) {
		...
		foreach ($settings as $field => $options) {
			...
			$options['dir_original_setup'] = $options['dir'] = rtrim($this->_replaceTokens($model, $options['dir'], $field, $tokens), DS);
			$options['uploadName'] = rtrim($this->_replaceTokens($model, $options['uploadName'], $field, $tokens), DS);
			$options['uploadNameFixed'] = rtrim($this->_replaceTokens($model, $options['uploadNameFixed'], $field, $tokens), DS);
			...
			if($options['createDirectory']) {
				...
				if (!empty($options['thumbsizes'])) {
					...
					$this->_createFolders($options['dir']);
				}
			}
...
/**
 * Checks if the aspectratio is allowed
 *
 * @param object $model Reference to model
 * @param array $data
 * @return boolean
 * @access public
 */
	function uploadCheckAspectRatio(&$model, $data) {
		return $this->_uploadCheckSize($model, $data, 'aspectRatio');
	}


	function uploadedFile(&$model, $data){
		foreach($data as $fieldName => $field){
			if (!$model->validate[$fieldName]['UploadedFile']['check'] || empty($field['tmp_name'])) {
				continue;
			}
	...
	function _uploadCheckSize(&$model, &$data, $type) {
		foreach ($data as $fieldName => $field) {
			...
			if (substr($type, 0, 3) === 'min') {
				...
				if(empty($options['length'][$type]))continue;
				
				if(!is_array($options['length'][$type])){
					$check = round($imgWidth/$imgHeight, strlen(preg_replace('/^[^\.]+\./', '', $options['length'][$type])));
					
					if (!empty($field['name']) && !empty($options['length'][$type]) && $check != $options['length'][$type]) {
						return false;
					}
					
				}else{
					$min = $options['length'][$type][0];
					$max = $options['length'][$type][1];
					
					$min_digits = min(strlen(preg_replace('/^[^\.]+\./', '', $min)), strlen(preg_replace('/^[^\.]+\./', '', $max)));
					if($min_digits > 0){
						$check = round($imgWidth/$imgHeight, $min_digits);
					}else{
						$check = $imgWidth/$imgHeight;
					}
					
					if (!empty($field['name']) && !empty($options['length'][$type]) && !$Validation->between($check, $min, $max)) {
						return false;
					}
				}
			}
 ...
 */
	function _uploadFile(&$model, $data = null) {
		...
		foreach ($this->__fields[$model->alias] as $fieldName => $options) {
			...
			//força o nome da imagem
			if(!empty($options['uploadNameFixed'])){
				$data[$model->alias][$fieldName]['name'] = $options['uploadNameFixed'].'.'.$ext;
			}
			...
			if ($options['useTable'] == false) {
				...
				if ($options['encryptedFolder']) {
					...
					//guarda o caminho para usar somente uma pasta por registro
					if(!isset($this->encryptedFolder))$this->encryptedFolder = String::uuid();
					$options['dir'] .= DS . date('Ym').DS.$this->encryptedFolder;
					$this->__fields[$model->alias][$fieldName]['dir'] = $options['dir'];
				}
...
/*
				if(!is_dir(dirname($saveAs))){
					$this->_createFolders(dirname($saveAs));
				}
*/
				...
				if(!empty($options['uploadNameIncludeDir'])){
					$data[$model->alias][$fieldName] = str_replace('\\', '/', preg_replace('/^'.preg_quote(realpath($options['dir_original_setup']), '/').'/', '', realpath(dirname($saveAs))).DS.basename($saveAs));
				}
				$result = array('return' => true, 'data' => $data);
				continue;
			}
 ...
 */
	function _setupValidation(&$model, $fieldName, $options) {
		...
		if (isset($model->validate[$fieldName])) {
			...
			foreach(array('MaxSize', 'MinWidth', 'MinHeight', 'MaxWidth', 'MaxHeight', 'AspectRatio') AS $validateRule){
				$validateLimit = strtolower(substr($validateRule, 0,1)).substr($validateRule, 1);
				
				if($validateRule == 'MaxSize'){
					$validateLimit = $this->_bytesToSize($options[$validateLimit]);
				}else if($validateRule != 'AspectRatio')
					$validateLimit = $options['length'][$validateLimit];
				else
				{
					//AspectRatio
					if(!is_array($options['length'][$validateLimit])){
						$validateLimit = $options['length'][$validateLimit];
					}else{
						$validateLimit = $options['length'][$validateLimit][0].' ~ '.$options['length'][$validateLimit][1];
					}
				}
				$model->validate[$fieldName][$validateRule]['message'] = sprintf($options['validations'][$validateRule]['message'], $validateLimit);
			}
 ...
 * @access protected
 */
	function _createFolders($dir, $thumbDir = null, $thumbsizes = null) {
		if ($dir[0] !== '/') {
			$dir = WWW_ROOT . $dir;
		}
		$folder = new Folder();
 ...
 */
	function _copyFileFromTemp($tmpName, $saveAs) {
		$results = true;
/*
		AÇÃO TRANSFORMADA EM VALIDAÇÃO
 ...
 */
	function _setFileToRemove(&$model, $fieldName, $thumbDir) {
		...
		if (!empty($filename) && $filename != $this->__fields[$model->alias][$fieldName]['default']) {
			...
			if(!empty($this->__fields[$model->alias][$fieldName]['uploadNameIncludeDir'])){
				if(!empty($this->__fields[$model->alias][$fieldName]['folderAsField']))$dir = dirname($dir);
				if(!empty($this->__fields[$model->alias][$fieldName]['encryptedFolder']))$dir = dirname(dirname($dir));
			}
 ...
 */
	function _deleteFiles(&$model, $field, $filename, $dir) {
		...
		$this->removeEmptyFolder(&$model, $field, $dir);
...
//Add dir
	function afterFind(&$model, $results){
		foreach ($this->__fields[$model->alias] as $fieldName => $options) {
			if(!is_null($options['folderAsField'])){
				foreach($results AS $key=>$r){
					if(!isset($r[$model->alias][$fieldName]) || !isset($r[$model->alias][$options['folderAsField']]))continue;
					$results[$key][$model->alias][$fieldName] = $options['url']. '/'. $r[$model->alias][$options['folderAsField']].'/'.$r[$model->alias][$fieldName];
				}
			}
		}
		return $results;
	...
	function removeEmptyFolder(&$model, $field, $path){
		$options = $this->__fields[$model->alias][$field];
		...
		foreach (glob($path.DIRECTORY_SEPARATOR."*") as $file){
			if (is_dir($file)){
				if (!$this->removeEmptyFolder(&$model, $field, $file)){
					$empty=false;
				}
			}
			else{
		...
		if ($empty && is_dir($path)){
			foreach(array(
				...
				//se é o caminho não é um subdiretório de configuração
				if(!preg_match('/^'.preg_quote( realpath($options['dir']).DIRECTORY_SEPARATOR, '/').'/', $dir)){
					continue;
				}
