...
class Jogador extends AppModel {
	...
	function setupAdmin($action = null, $id = null){
		...
		$countries = Set::combine(WebjumpUtilities::countries(), '{s}.2', '{s}.0');
...
//				$this->alias.'.id' => 'Código',
	...
	function afterFindExecute($result){
		...
		if(isset($result['pais'])){
			App::import('Vendor', 'WebjumpUtilities');
			$countries = Set::combine(WebjumpUtilities::countries(), '{s}.2', '{s}.1');
			$result['pais_iso_2'] = ($result['pais'] == '000' || empty($result['pais']) ? '00' : strtolower($countries[$result['pais']]));
		}
